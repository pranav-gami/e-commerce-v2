...
const handleMoveToCart = async (e, product) => {
        ...
        {wishlistItems.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center bg-white rounded shadow-sm">
            <div className="w-20 h-20 bg-primary-light rounded-full flex items-center justify-center mb-5">
              <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#ff3f6c" strokeWidth="1.5">
                <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" />
              </svg>
            </div>
            <h2 className="text-xl font-extrabold text-brand-dark">Your wishlist is empty</h2>
            <p className="text-brand-gray mt-1 text-sm">Save items you love by clicking the heart icon.</p>
            <Link to="/products"
  ...
  className={`group bg-white border border-brand-border overflow-hidden hover:shadow-md transition-all duration-300 relative ${isMoved ? "opacity-50 scale-95" : ""}`}>
                  ...
                  <button
                    ...
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="#ff3f6c" stroke="#ff3f6c" strokeWidth="1.5">
                      <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" />
                    </svg>
                  </button>

                  {/* Image */}
                  <div onClick={(e) => { e.stopPropagation(); navigate(`/products/${product.id}`); }}
...
onClick={(e) => handleMoveToCart(e, product)}
                      disabled={!inStock || addingId === product.id || isMoved}
                      ...
                      {isMoved ? "MOVED!" : addingId === product.id ? "MOVING..." : inStock ? "MOVE TO BAG" : "OUT OF STOCK"}
                    </button>
                  </div>
                </div>
              );

...
const ProfilePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true); setServerError(""); setLocationErrors({});
      const fullPhone = `${locationIds.phoneCode}${locationIds.phone}`;
      await updateProfile({
        name: form.name, phone: fullPhone, address: form.address,
        countryId: locationIds.countryId, stateId: locationIds.stateId,
        cityId: locationIds.cityId, postalCode: locationIds.postalCode,
      });
  ...
  const handleCancel = () => {
    setIsEditing(false); setServerError(""); setLocationErrors({});
    if (user) {
      const storedPhone = user.phone || "";
      let phoneCode = user.country?.phoneCode || "";
      let phoneNumber = storedPhone;
      if (phoneCode && storedPhone.startsWith(phoneCode)) phoneNumber = storedPhone.slice(phoneCode.length);
      setForm({ name: user.name || "", address: user.address || "" });
      setLocationIds({
        countryId: user.country?.id ?? null, stateId: user.state?.id ?? null,
        cityId: user.city?.id ?? null, postalCode: user.postalCode ?? "",
  ...
  return (
    <div className="min-h-screen bg-brand-light py-10 px-4">
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        <div className="bg-white border border-brand-border rounded shadow-sm overflow-hidden">
          ...
          <div className="p-6 flex items-center gap-5">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="text-lg font-extrabold text-brand-dark truncate">{user?.name}</h2>
            ...
            {!isEditing && (
              <button onClick={() => setIsEditing(true)}
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <line x1="18" y1="2" x2="22" y2="6" /><path d="M7.5 20.5L2 22l1.5-5.5L16 4l4 4L7.5 20.5z" />
                </svg>
                EDIT
              </button>
            )}
        ...
        {/* Alerts */}
        {serverError && (
          <div className="bg-red-50 border border-red-200 text-red-600 text-sm px-4 py-3 rounded font-medium">{serverError}</div>
        )}
        {successMessage && (
        ...
        <div className="bg-white border border-brand-border rounded shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-brand-border">
            <h3 className="text-xs font-extrabold text-brand-gray uppercase tracking-wider">Personal Information</h3>
          </div>
          ...
          {!isEditing ? (
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div>
                ...
                <input type="text" name="address" value={form.address} onChange={handleChange} placeholder="House/Flat, Street, Area"
                  className="w-full border border-brand-border rounded px-4 py-3 text-sm text-brand-dark placeholder-brand-gray focus:outline-none focus:border-primary transition-colors" />
              </div>

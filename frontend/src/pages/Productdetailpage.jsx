...
const ProductDetailPage = () => {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                ...
                setActiveImg(0);
                setImgError(false);
                const simRes = await api.get(`/products?subCategoryId=${p.subCategoryId}&limit=7`);
    ...
    const handleAddCart = async () => {
        ...
        try {
            ...
            setCartMsg(
                typeof err === 'string' ? err : err?.response?.data?.message || 'Failed to add',
            );
        } finally {
            setAddingCart(false);
        }
    ...
    return (
        <div className="bg-white min-h-screen pb-20">
            ...
            <section className="max-w-screen-xl mx-auto px-4 py-8">
                <div className="flex flex-col lg:flex-row gap-10">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-5 flex gap-3 flex-wrap hidden sm:flex">
                            {' '}
                            {cartQty > 0 ? (
            ...
            <div className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-brand-border p-3 flex gap-3 sm:hidden">
                {cartQty > 0 ? (
                    <div className="flex items-center border-2 border-primary rounded-sm overflow-hidden flex-1">
                        <button
                            onClick={handleCartDec}
                            className="w-10 h-10 flex items-center justify-center text-primary font-bold text-lg"
                        >
                            −
                        </button>
                        <span className="flex-1 text-center text-primary font-bold">{cartQty}</span>
                        <button
                            onClick={handleCartInc}
                            disabled={cartQty >= product.stock}
                            className="w-10 h-10 flex items-center justify-center text-primary font-bold text-lg disabled:opacity-40"
                        >
                            +
                        </button>
                    </div>

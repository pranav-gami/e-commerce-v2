...
//       await sleep(800); // avoid blocking
//     } catch (err) {
//       failed++;
    ...
    try {
      await new Promise((resolve, reject) => {
        ...
        const req = client.get(url, (res) => {
          // ✅ FIX: handle redirect
          if ([301, 302].includes(res.statusCode)) {
            return downloadImage(res.headers.location, filename, retries)
              .then(resolve)
              .catch(reject);
          }
...
async function main() {
  ...
  const toFix = products.filter((p) => {
    ...
    // only local images
    if (!p.image.startsWith("/uploads/")) return false;
  ...
  for (let i = 0; i < toFix.length; i++) {
    ...
    try {
      ...
      const existingPath = p.image
        ? path.join(UPLOAD_DIR, path.basename(p.image))
        : null;

      // 🛑 Skip if already valid
      if (existingPath && isValidImage(existingPath)) {
        console.log("⏭️ Skipped (already valid)");
        continue;
      }

      // ✅ Always working image source

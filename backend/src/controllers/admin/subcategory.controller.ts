...
export const getSubCategoriesPage = catchAsyncHandler(async (req: AuthRequest, res: Response) => {
  if (req.headers.accept?.includes("application/json")) {
    const { categoryId } = req.query;
    const subs = await subCategoryService.getAllSubCategories(categoryId ? Number(categoryId) : undefined);
    return res.json({ success: true, data: subs });
  }
  const admin = await adminService.getCurrentAdmin(req.user?.id!);
  const categories = await categoryService.getAllCategories();
  res.render("pages/subcategories", { page: "subcategories", title: "Sub Categories", admin, categories });
});

export const createSubCategory = catchAsyncHandler(async (req: Request, res: Response) => {
  const { name, description, categoryId } = req.body;
  if (!name) throw new ApiError(400, "Name is required");
  if (!categoryId) throw new ApiError(400, "categoryId is required");
  const data = await subCategoryService.createSubCategory({ name, description, categoryId: Number(categoryId) });
...
export const updateSubCategory = catchAsyncHandler(async (req: Request, res: Response) => {
  const { name, description, categoryId } = req.body;
  const data = await subCategoryService.updateSubCategory(Number(req.params.id), {
    name,
    description,
...
export const deleteSubCategory = catchAsyncHandler(async (req: Request, res: Response) => {
  const data = await subCategoryService.deleteSubCategory(Number(req.params.id));
  return sendResponse(res, 200, "Sub-category deleted successfully", data);
});

export const bulkDeleteSubCategories = catchAsyncHandler(async (req: Request, res: Response) => {
  const { ids } = req.body;
  ...
  return sendResponse(res, 200, `${numericIds.length} sub-categories deleted successfully`, null);
});
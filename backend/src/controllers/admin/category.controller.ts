...
export const getCategoriesPage = catchAsyncHandler(
...
export const getAddCategoryPage = catchAsyncHandler(
  async (req: AuthRequest, res: Response) => {
    const admin = await adminService.getCurrentAdmin(req.user?.id!);
...
export const getEditCategoryPage = catchAsyncHandler(
  async (req: AuthRequest, res: Response) => {
    const admin = await adminService.getCurrentAdmin(req.user?.id!);
...
export const getAllCategories = catchAsyncHandler(
  async (req: Request, res: Response) => {
...
export const updateCategory = catchAsyncHandler(
  async (req: Request, res: Response) => {
    const { name, description } = req.body;
    const data = await categoryService.updateCategory(Number(req.params.id), {
      name,
      description,
      file: req.file,
    });
...
export const bulkDeleteCategories = catchAsyncHandler(
  async (req: Request, res: Response) => {
    const { ids } = req.body;
    ...
    return sendResponse(
      res,
      200,
      `${numericIds.length} categories deleted successfully`,
      null,
    );
  },
);

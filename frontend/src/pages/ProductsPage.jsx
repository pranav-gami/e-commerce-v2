...
import { selectSearchQuery, setSearchQuery, clearSearch } from "../redux/slices/searchSlice";
...
<<<<<<< HEAD
                p === "..." ? (
                    <span key={`d${i}`} className="w-8 h-8 flex items-center justify-center text-[#94969f] text-sm">…</span>
=======
                p === '...' ? (
                    <span
                        key={`d${i}`}
...
>>>>>>> 95c69cb0528cc8bbd2f1eceea3cab1b82d5206c4
                    ...
                    <button
                        ...
                        onClick={() => onPage(p)}
<<<<<<< HEAD
                        className={`w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center border text-sm font-bold transition-colors ${p === page ? "bg-[#ff3f6c] text-white border-[#ff3f6c]" : "bg-white border-[#d4d5d9] text-[#282c3f] hover:border-[#ff3f6c] hover:text-[#ff3f6c]"}`}
=======
...
const SortDropdown = ({ sortBy, setSortBy, upward = false }) => {
=======
//Sort Dropdown
const SORT_OPTIONS = [
    { value: '', label: 'Recommended' },
...
<<<<<<< HEAD
    ...
    return (
        <div ref={ref} className="relative flex-1">
            <button
                type="button"
                className="w-full flex items-center justify-center gap-2 py-2.5 text-[13px] font-semibold text-[#282c3f] bg-white transition-colors"
                onClick={() => setOpen((o) => !o)}
            >
                ...
                <span>Sort</span>
                {sortBy && <span className="text-[#ff3f6c] text-[11px] font-bold">•</span>}
            </button>
            {open && (
                <div className={`absolute left-0 z-50 bg-white border border-[#d4d5d9] shadow-lg w-[200px] py-1 ${upward ? "bottom-full mb-1" : "top-full mt-1"}`}>
                    {SORT_OPTIONS.map((opt) => (
                        <button
                            key={opt.value}
                            type="button"
                            onClick={() => { setSortBy(opt.value); setOpen(false); }}
                            className={`w-full text-left px-4 py-2.5 text-[13px] transition-colors ${sortBy === opt.value ? "bg-[#fff0f3] text-[#ff3f6c] font-semibold" : "text-[#282c3f] hover:bg-[#f5f5f6]"}`}
                        >
                            {opt.label}
                            {sortBy === opt.value && <span className="float-right text-[#ff3f6c]">✓</span>}
                        </button>
                    ))}
                </div>
            )}
...
// ─── Desktop Sort Dropdown (original style) ───────────────────────────────────
const DesktopSortDropdown = ({ sortBy, setSortBy }) => {
    const [open, setOpen] = useState(false);
    const ref = useRef(null);
...
<<<<<<< HEAD
                    ...
                    <button onClick={onClose} className="text-[#94969f] hover:text-[#282c3f] bg-transparent border-none cursor-pointer">
...
>>>>>>> 95c69cb0528cc8bbd2f1eceea3cab1b82d5206c4
                                ...
                                />
<<<<<<< HEAD
                                <span className="text-[13px] text-[#282c3f] group-hover:text-[#ff3f6c] transition-colors truncate">{item.name}</span>
                            </label>
                        ))}
                        {filtered.length === 0 && (
...
<<<<<<< HEAD
                    ...
                    <button onClick={onClose} className="bg-[#ff3f6c] text-white px-6 py-2 text-[13px] font-bold hover:bg-[#e8365d] transition-colors border-none cursor-pointer">
=======
...
>>>>>>> 95c69cb0528cc8bbd2f1eceea3cab1b82d5206c4
                ...
                step={1}
<<<<<<< HEAD
                onChange={(_, v) => { setIsDragging(true); setLocal(v); }}
...
<<<<<<< HEAD
            ...
            className="fixed bottom-20 right-4 z-50 w-10 h-10 bg-[#ff3f6c] text-white rounded-full shadow-lg flex items-center justify-center hover:bg-[#e8365d] transition-all hover:scale-110 active:scale-95 border-none cursor-pointer"
        >
...
const Breadcrumb = ({ categories, selectedCategory, selectedSubcategory, subcategories, searchQuery }) => {
    const cat = selectedCategory.length ? categories.find((c) => String(c.id) === String(selectedCategory[0])) : null;
    const sub = selectedSubcategory.length ? subcategories.find((s) => String(s.id) === String(selectedSubcategory[0])) : null;
    const Sep = () => (
...
<<<<<<< HEAD
            ...
            <Sep />
            <Link to="/products" className="hover:text-[#ff3f6c] transition-colors">Products</Link>
            {cat && (<><Sep /><Link to={`/products?category=${cat.id}`} className="hover:text-[#ff3f6c] transition-colors">{cat.name}</Link></>)}
            {sub && (<><Sep /><span className="font-semibold text-[#282c3f]">{sub.name}</span></>)}
            {!cat && searchQuery && (<><Sep /><span className="font-semibold text-[#282c3f]">"{searchQuery}"</span></>)}
=======
...
const FilterPill = ({ label, onRemove }) => (
    <span className="flex items-center gap-1.5 border border-[#d4d5d9] rounded-full text-[#282c3f] text-[11px] sm:text-[12px] font-semibold px-2.5 py-1 bg-white hover:border-[#ff3f6c] transition-colors">
        {label}
        <button onClick={onRemove} className="text-[#94969f] hover:text-[#ff3f6c] font-bold leading-none bg-transparent border-none cursor-pointer p-0">×</button>
    </span>
);
...
<<<<<<< HEAD
            selectedCategory.map((catId) =>
                api.get(`/subcategories?categoryId=${catId}`).then((res) => res.data.data?.subCategories || []),
            ),
        ).then((results) => {
            const seen = new Set();
            const unique = results.flat().filter((s) => { if (seen.has(s.id)) return false; seen.add(s.id); return true; });
            setSubcategories(unique);
            setSelectedSubcategory((prev) => prev.filter((id) => unique.some((s) => String(s.id) === String(id))));
        }).catch(console.error);
    }, [selectedCategory]);
...
<<<<<<< HEAD
    ...
    }, [selectedCategory, selectedSubcategory, searchQuery, priceRange, minDiscount, discountOnly, sortBy, dispatch]);
=======
    //Load products
...
>>>>>>> 95c69cb0528cc8bbd2f1eceea3cab1b82d5206c4
    ...
    useEffect(() => {
        ...
        loadProducts(1);
<<<<<<< HEAD
    }, [selectedCategory, selectedSubcategory, searchQuery, priceRange.min, priceRange.max, minDiscount, discountOnly, sortBy]);
...
<<<<<<< HEAD
    const hasActiveFilters = selectedCategory.length || selectedSubcategory.length || priceRange.min || priceRange.max || discountOnly || minDiscount !== null || searchQuery;
=======
...
<<<<<<< HEAD
        ...
        discountOnly, minDiscount !== null, searchQuery,
...
<<<<<<< HEAD
                <h3 className="text-[15px] font-semibold text-[#282c3f] uppercase tracking-[0.8px] m-0">Filters</h3>
                {hasActiveFilters && (
                    <button onClick={handleClearFilters} className="text-[13px] font-semibold text-[#ff3f6c] hover:underline uppercase tracking-wide bg-transparent border-none cursor-pointer">
=======
                ...
                <div className="space-y-2.5">
                    {categories.slice(0, 8).map((cat) => (
                        <label key={cat.id} className="flex items-center gap-2.5 cursor-pointer group">
                            <input type="checkbox" checked={selectedCategory.includes(String(cat.id))} onChange={() => toggleValue(String(cat.id), setSelectedCategory)} className="accent-[#ff3f6c] w-3.5 h-3.5" />
                            <span className="text-[13px] text-[#282c3f] group-hover:text-[#ff3f6c] transition-colors">{cat.name}</span>
                        </label>
                    ))}
                    {categories.length > 8 && (
                        <button onClick={() => setCategoryModalOpen(true)} className="text-[13px] text-[#ff3f6c] font-semibold hover:underline bg-transparent border-none cursor-pointer p-0">
=======
            {/* Category */}
            <FilterSection title="Category">
                <div className="space-y-2.5">
                    ...
                    <div className="space-y-2.5">
                        {subcategories.slice(0, 8).map((sub) => (
                            <label key={sub.id} className="flex items-center gap-2.5 cursor-pointer group">
                                <input type="checkbox" checked={selectedSubcategory.includes(String(sub.id))} onChange={() => toggleValue(String(sub.id), setSelectedSubcategory)} className="accent-[#ff3f6c] w-3.5 h-3.5" />
                                <span className="text-[13px] text-[#282c3f] group-hover:text-[#ff3f6c] transition-colors">{sub.name}</span>
                            </label>
                        ))}
                        {subcategories.length > 8 && (
                            <button onClick={() => setSubcategoryModalOpen(true)} className="text-[13px] text-[#ff3f6c] font-semibold hover:underline bg-transparent border-none cursor-pointer p-0">
=======
            {/* Subcategory */}
            {subcategories.length > 0 && (
                <FilterSection title="Subcategory">
...
<<<<<<< HEAD
            <FilterSection title={`Price Range (₹${catMinPrice.toLocaleString("en-IN")} – ₹${catMaxPrice.toLocaleString("en-IN")})`}>
                <PriceSlider key={sliderKey} catMinPrice={catMinPrice} catMaxPrice={catMaxPrice} priceRange={priceRange} onCommit={setPriceRange} />
=======
                ...
                <div className="space-y-2.5">
                    {availableDiscounts.length > 0 ? (
                        availableDiscounts.map((pct) => {
                            ...
                            return (
                                <label key={pct} className="flex items-center gap-2.5 cursor-pointer group" onClick={() => setMinDiscount(isSelected ? null : pct)}>
                                    <span className={`w-3.5 h-3.5 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-colors ${isSelected ? "border-[#ff3f6c] bg-[#ff3f6c]" : "border-[#94969f] bg-white group-hover:border-[#ff3f6c]"}`}>
                                        {isSelected && <span className="w-1.5 h-1.5 rounded-full bg-white block" />}
                                    </span>
                                    <span className="text-[13px] text-[#282c3f] group-hover:text-[#ff3f6c] transition-colors select-none">{pct}% and above</span>
=======
...
<<<<<<< HEAD
                            {selectedCategory.length > 0 ? "No discounts available in this category" : "Select a category to see discounts"}
=======
                            {selectedCategory.length > 0
...
<<<<<<< HEAD
            <FilterSection title="Offers">
                <label className="flex items-center gap-2.5 cursor-pointer group">
                    <input type="checkbox" checked={discountOnly} onChange={(e) => setDiscountOnly(e.target.checked)} className="accent-[#ff3f6c] w-3.5 h-3.5" />
                    <span className="text-[13px] text-[#282c3f] group-hover:text-[#ff3f6c] transition-colors">Discounted Only</span>
=======
            {/* Offers */}
            <FilterSection title="Offers">
                <label className="flex items-center gap-2.5 cursor-pointer group">
...
>>>>>>> 95c69cb0528cc8bbd2f1eceea3cab1b82d5206c4
    ...
    return (
        <div className="bg-white min-h-screen">
            ...
            {categoryModalOpen && (
<<<<<<< HEAD
                <MoreModal title="Category" items={categories} selectedItems={selectedCategory} onToggle={(id) => toggleValue(id, setSelectedCategory)} onClose={() => setCategoryModalOpen(false)} />
            )}
            {subcategoryModalOpen && (
                <MoreModal title="Subcategory" items={subcategories.map((s) => ({ ...s, id: String(s.id) }))} selectedItems={selectedSubcategory} onToggle={(id) => toggleValue(id, setSelectedSubcategory)} onClose={() => setSubcategoryModalOpen(false)} />
            )}

            <div className="w-full px-3 sm:px-5 lg:px-10 xl:px-14 py-4 sm:py-5 pb-20 lg:pb-5">
                <Breadcrumb categories={categories} selectedCategory={selectedCategory} selectedSubcategory={selectedSubcategory} subcategories={subcategories} searchQuery={searchQuery} />
                ...
                <div className="flex items-baseline gap-2 flex-wrap mb-4 sm:mb-5">
                    ...
                    {!loading && pagination && (
                        <span className="text-[14px] sm:text-[17px] text-[#94969f]">— {pagination.total} items</span>
                    )}
                </div>

                {/* Active filter pills — mobile only, above grid */}
                {hasActiveFilters && (
                    <div className="lg:hidden flex flex-wrap items-center gap-1.5 mb-3">
                        {selectedCategory.map((catId) => {
                            const cat = categories.find((c) => String(c.id) === String(catId));
                            return cat ? <FilterPill key={catId} label={cat.name} onRemove={() => toggleValue(catId, setSelectedCategory)} /> : null;
                        })}
                        {selectedSubcategory.map((subId) => {
                            const sub = subcategories.find((s) => String(s.id) === String(subId));
                            return sub ? <FilterPill key={subId} label={sub.name} onRemove={() => toggleValue(String(subId), setSelectedSubcategory)} /> : null;
                        })}
                        {discountOnly && <FilterPill label="Offers" onRemove={() => setDiscountOnly(false)} />}
                        {minDiscount !== null && <FilterPill label={`${minDiscount}% and above`} onRemove={() => setMinDiscount(null)} />}
                        {(priceRange.min || priceRange.max) && (
                            <FilterPill label={`₹${priceRange.min || "0"} – ₹${priceRange.max || "∞"}`} onRemove={() => { setPriceRange({ min: "", max: "" }); setSliderKey((k) => k + 1); }} />
                        )}
                        {searchQuery && <FilterPill label={`"${searchQuery}"`} onRemove={() => dispatch(setSearchQuery(""))} />}
                    </div>
                )}
...
<<<<<<< HEAD
                        <div className="sticky top-16 max-h-[calc(100vh-4rem)] overflow-y-auto pr-2" style={{ scrollbarWidth: "none" }}>
=======
                        <div
...
>>>>>>> 95c69cb0528cc8bbd2f1eceea3cab1b82d5206c4
                    ...
                    {/* Mobile sidebar */}
<<<<<<< HEAD
                    <aside className={`lg:hidden fixed inset-y-0 left-0 z-50 w-[280px] sm:w-[320px] bg-white overflow-y-auto transition-transform duration-300 shadow-2xl ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
                        <div className="flex items-center justify-between p-4 border-b border-[#eaeaec] sticky top-0 bg-white z-10">
                            ...
                            <button onClick={() => setSidebarOpen(false)} className="bg-transparent border-none cursor-pointer p-1">
...
<<<<<<< HEAD
                        ...
                        {/* Desktop toolbar — sort + filter pills */}
                        <div className="hidden lg:flex items-start justify-between gap-4 flex-wrap mb-2">
                            {hasActiveFilters && (
...
>>>>>>> 95c69cb0528cc8bbd2f1eceea3cab1b82d5206c4
                        ...
                        {loading ? (
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 mt-3">
                                {[...Array(LIMIT)].map((_, i) => (
<<<<<<< HEAD
                                    <div key={i} className="bg-[#f5f5f6] animate-pulse rounded-sm" style={{ aspectRatio: "3/4" }} />
=======
                                    <div
                                        key={i}
...
<<<<<<< HEAD
                                <svg width="56" height="56" viewBox="0 0 24 24" fill="none" stroke="#94969f" strokeWidth="1.2">
                                    <circle cx="11" cy="11" r="8" /><path d="m21 21-4.35-4.35" />
                                </svg>
                                <h2 className="text-lg sm:text-xl font-bold text-[#282c3f] mt-4">No products found</h2>
                                <p className="text-[#94969f] mt-1 text-sm">Try adjusting your filters or search term</p>
                                <button onClick={handleClearFilters} className="mt-5 bg-[#ff3f6c] text-white px-6 py-2.5 text-sm font-bold hover:bg-[#e8365d] transition-colors border-none cursor-pointer">
=======
                                <svg
                                    width="56"
                                    height="56"
...
<<<<<<< HEAD
            ...
            <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-[#eaeaec] shadow-[0_-2px_8px_rgba(0,0,0,0.08)] flex">
                {/* Sort button */}
                <SortDropdown sortBy={sortBy} setSortBy={setSortBy} upward />

                {/* Divider */}
                <div className="w-px bg-[#eaeaec] self-stretch" />

                {/* Filter button */}
                <button
                    onClick={() => setSidebarOpen(true)}
                    className="flex-1 flex items-center justify-center gap-2 py-2.5 text-[13px] font-semibold text-[#282c3f] bg-white"
                >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    ...
                    <span>Filter</span>
                    {activeFilterCount > 0 && (
                        <span className="bg-[#ff3f6c] text-white text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center leading-none">
                            {activeFilterCount}
                        </span>
                    )}
=======

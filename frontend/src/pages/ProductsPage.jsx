...
// ─── Sort Dropdown (click-based only) ────────────────────────────────────────
const SORT_OPTIONS = [
...
const SortDropdown = ({ sortBy, setSortBy }) => {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <span>
          <span className="text-[#94969f] font-normal hidden sm:inline">
            Sort by:{" "}
          </span>
...
const ProductsPage = () => {
  ...
  // ── Filter meta — dynamic from backend ────────────────────────────────────
  const [catMinPrice, setCatMinPrice] = useState(0);
  const [catMaxPrice, setCatMaxPrice] = useState(8000);
  const [availableDiscounts, setAvailableDiscounts] = useState([
    10, 20, 30, 40, 50,
  ]);
  // sliderKey forces PriceSlider to remount (reinitialise) when bounds change
  ...
  // This makes the price slider and discount options fully dynamic.
  useEffect(() => {
    ...
    api
      .get(`/products/filter-meta?${query.toString()}`)
      .then((res) => {
        const {
          minPrice,
          maxPrice,
        ...
        setAvailableDiscounts(
          discounts?.length ? discounts : [10, 20, 30, 40, 50],
        );
      ...
      .catch(() => {
        // silent fallback — keep previous values
      });
  ...
  const categoryName = useMemo(() => {
    ...
    return selectedCategory
      .map(
        (id) => categories.find((c) => String(c.id) === String(id))?.name || "",
      )
      .filter(Boolean)
      .join(", ");
  ...
  useEffect(() => {
    ...
    if (categoryName)
      title = `${categoryName} | Buy ${categoryName} Online | Shop.in`;
      ...
      >
        ...
        {(priceRange.min || priceRange.max) && (
          <p className="text-[11px] text-[#ff3f6c] mt-2 font-semibold">
            ₹{priceRange.min || catMinPrice} – ₹{priceRange.max || catMaxPrice}{" "}
            applied
          </p>
        )}
      </FilterSection>
                  ...
                  {selectedCategory.map((catId) => {
                    ...
                    return cat ? (
                      <FilterPill
                        key={catId}
                        label={cat.name}
                        onRemove={() => toggleValue(catId, setSelectedCategory)}
                      />
                    ) : null;
                  })}
